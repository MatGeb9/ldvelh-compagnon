...
└── js/
    ...
    ├── map.js             ← mind-map SVG : Fruchterman-Reingold + pan/pinch-zoom + nœuds épinglables
...
L'écran de jeu a 5 onglets : **Personnage** (stats + paragraphe + dés log + alertes), **Combat**, **Inventaire**, **Notes** (notes libres + historique paragraphes filtrable), **Carte** (mind-map SVG).
...
```js
{
  ...
  alerts: [{ text, type: 'warning'|'info'|'danger'|'success' }],
  notes: '...texte libre...',

  currentParagraph: 42,
  paragraphHistory: [1, 2, 5, null, 1, 3, ...],  // null = séparateur entre 2 runs
  paragraphs: { '1': { sentiment: 'neutral'|'positive'|'negative', note: '' }, ... },

  diceLog: [{ rolls, modifier, modifierLabel, total, label, ts }, ...],  // borné à 10
  mapLockedPositions: { '42': { x, y } },  // positions manuelles épinglées par drag sur la carte
...
### Paragraphes & carte
- Bouton **+ Ajouter** = visite (push dans `paragraphHistory`, crée une arête sur la carte).
- Bouton **+ Voir seulement** = ajoute le nœud sur la carte sans le compter comme visite (pratique pour repérer un § avant d'y aller).
- Sentiments : neutre (gris), positif (vert, bénéfique), négatif (rouge, piège). Cliquer le rond cycle.
- Carte : layout force-directed (Fruchterman-Reingold), drag-pour-épingler une position, pinch/molette = zoom. Positions épinglées persistées dans `mapLockedPositions`.
...
- Toucher à `paragraphHistory` sans gérer les `null` (marqueurs de run) → casse `renderParagraphs` et `buildGraph` (map).
